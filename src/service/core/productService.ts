...
export const productsService = {
  ...
  getByWarehouse: async (
    ...
    try {
      const res: AxiosResponse<Product[]> = await axiosClient.get(
        `/products/warehouse/${warehouseCode}`,
        {
          signal,
        }
      );
      ...
      // Manejo específico del error 404 (No se encontraron productos)
      if (axiosError.response?.status === 404) {
        const errorMessage =
          axiosError.response?.data?.message ||
          `No se encontraron productos para la bodega '${warehouseCode}'`;

        // Puedes decidir si mostrar o no la notificación para 404
        // En este caso, NO mostramos notificación y dejamos que el componente maneje el estado vacío
        throw new Error(errorMessage);
      }
      ...
      const errorMessage =
        axiosError.response?.data?.message ||
        "Error al cargar productos de la bodega";
